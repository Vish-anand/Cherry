document.addEventListener("DOMContentLoaded", () => {
    ...
    // Sanitizes an HTML string before it is ever assigned to innerHTML. Chat content can
    // transitively include untrusted text (scraped web pages, uploaded document contents,
    // inbound WhatsApp messages) — if DOMPurify hasn't loaded for some reason, fail safe by
    // stripping all markup entirely rather than inserting unsanitized HTML.
    function sanitizeHtml(html) {
        if (typeof DOMPurify !== "undefined" && typeof DOMPurify.sanitize === "function") {
            return DOMPurify.sanitize(html, { ADD_ATTR: ["target"] });
        }
        console.error("DOMPurify is not loaded — rendering as plain text instead of HTML.");
    ...
    async function sendChatMessage() {
        ...
        if (voiceChatToggle && voiceChatToggle.checked) {
            url += `&voice_mode=true`;
            if (voiceModelSelect) {
                url += `&voice_model=${encodeURIComponent(voiceModelSelect.value)}`;
            }
        }
    ...
    function appendMessage(role, content, filename = null) {
        ...
        // `content` can transitively include untrusted text (scraped web pages, document
        // contents, WhatsApp messages) that the model echoed back — sanitize the rendered
        // HTML before it ever touches innerHTML. sanitizeHtml() falls back to a plain-text
        // rendering if DOMPurify hasn't loaded for some reason (see helper below).
        const finalHtml = `
        ...
        `;
        msgDiv.innerHTML = sanitizeHtml(finalHtml);
    ...
    async function sendWaMessage() {
        ...
        // Escape (not just sanitize) plain-text fields here — a chat bubble should never
        // render markup at all, so treat text/filename as literal text rather than HTML.
        // The blob: URL below is generated by the browser itself (createObjectURL), not
        // attacker-influenceable, so it's safe to interpolate directly.
        if (activeWaFile) {
            if (activeWaFile.type.startsWith("image/")) {
                const url = URL.createObjectURL(activeWaFile);
                inboundDiv.innerHTML = `<img src="${url}"><p>${escapeHtml(text)}</p>`;
            } else {
                inboundDiv.innerHTML = `<p><i class="fa-solid fa-file"></i> ${escapeHtml(activeWaFile.name)}</p><p>${escapeHtml(text)}</p>`;
            }
        } else {
            inboundDiv.innerHTML = `<p>${escapeHtml(text)}</p>`;
        }
        ...
        try {
            ...
            outboundDiv.innerHTML = `<p>${escapeHtml(data.agent_reply || "Webhook payload successfully parsed.")}</p>`;
    ...
    const voiceSamples = {
        'en-US-AnaNeural': "Hello! I am Ana. I speak in a natural and friendly American accent. How can I help you today?",
        'en-US-GuyNeural': "Hey there! I am Guy. I provide a clear, conversational American male voice. What shall we do?",
        'en-US-JennyNeural': "Hello! I am Jenny. This is a preview of my realistic American voice model.",
        'en-US-AriaNeural': "Greetings! I am Aria. Let me know if you need any system command executed or folders backed up.",
        'en-US-ChristopherNeural': "Hi there, I am Christopher. This is a demonstration of my US male vocal capabilities.",
        'en-GB-SoniaNeural': "Cheerio! I am Sonia. I speak with a realistic British female tone.",
        'en-GB-RyanNeural': "Hello there. I am Ryan, representing a professional United Kingdom male voice.",
        'en-IN-NeerjaNeural': "Namaste. I am Neerja, speaking in a warm, natural Indian English voice.",
        'en-IN-PrabhatNeural': "Hello, I am Prabhat. I provide a natural Indian English male tone.",
        'en_US-amy-medium': "Hello. I am Amy. I speak using a local offline voice model. No internet connection is needed for me to talk.",
        'en_US-kristin-medium': "Hello there, I am Kristin. This is an offline preview of my local medium quality speech engine.",
        'en_GB-jenny_dioco-medium': "Ello! I am Jenny Dioco. I speak with a British accent using an offline speech synthesis model.",
        'en_US-ryan-medium': "Hi, I am Ryan. I am a local male voice model designed to work completely offline.",
        'en_US-joe-medium': "Greetings, I am Joe. I speak using a local offline speech synthesis model."
    };

    const voiceModelSelect = document.getElementById("voice-model-select");
    const voiceSampleBtn = document.getElementById("voice-sample-btn");
    const localScannedVoices = document.getElementById("local-scanned-voices");
    const customVoiceInputContainer = document.getElementById("custom-voice-input-container");
    const customVoiceModelInput = document.getElementById("custom-voice-model-input");
    const applyCustomVoiceBtn = document.getElementById("apply-custom-voice-btn");
    const toggleCustomVoiceBtn = document.getElementById("toggle-custom-voice-btn");
    
    const voiceOnnxUpload = document.getElementById("voice-onnx-upload");
    const voiceJsonUpload = document.getElementById("voice-json-upload");
    const uploadVoiceBtn = document.getElementById("upload-voice-btn");
    const uploadVoiceStatus = document.getElementById("upload-voice-status");

    async function fetchLocalModels() {
        if (!localScannedVoices) return;
        try {
            const res = await fetch("/api/voice/local_models");
            const models = await res.json();
            localScannedVoices.innerHTML = "";
            if (Array.isArray(models)) {
                models.forEach(model => {
                    const opt = document.createElement("option");
                    opt.value = model;
                    opt.textContent = `${model} (Scanned Local)`;
                    localScannedVoices.appendChild(opt);
                });
            }
        } catch (err) {
            console.error("Error fetching local voice models:", err);
        }
    }

    async function loadVoiceConfig() {
        try {
            // First fetch local scanned models
            ...
            if (data && data.voice_model && voiceModelSelect) {
                // Check if options contains the model
                let hasOption = false;
                for (let i = 0; i < voiceModelSelect.options.length; i++) {
                    if (voiceModelSelect.options[i].value === data.voice_model) {
                        hasOption = true;
                        break;
                    }
                }
                if (hasOption) {
                    voiceModelSelect.value = data.voice_model;
                } else {
                    // Create option dynamically
                    const opt = document.createElement("option");
                    opt.value = data.voice_model;
                    opt.textContent = `${data.voice_model} (Custom)`;
                    voiceModelSelect.appendChild(opt);
                    voiceModelSelect.value = data.voice_model;
                    
                    if (customVoiceInputContainer) customVoiceInputContainer.style.display = "flex";
                    if (customVoiceModelInput) customVoiceModelInput.value = data.voice_model;
                }
            }
    ...
    if (voiceModelSelect) {
        voiceModelSelect.addEventListener("change", async () => {
            ...
            const voice_model = voiceModelSelect.value;
            try {
                await fetch("/api/voice/config", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ voice_api_url, voice_model })
                });
            } catch (err) {
                console.error("Error saving voice model change:", err);
            }
        });
    }

    if (toggleCustomVoiceBtn) {
        toggleCustomVoiceBtn.addEventListener("click", () => {
            if (customVoiceInputContainer) {
                const isHidden = customVoiceInputContainer.style.display === "none";
                customVoiceInputContainer.style.display = isHidden ? "flex" : "none";
            }
        });
    }
    ...
    if (applyCustomVoiceBtn) {
        applyCustomVoiceBtn.addEventListener("click", async () => {
            const voice_api_url = voiceApiUrlInput.value.trim();
            const voice_model = customVoiceModelInput.value.trim();
            if (!voice_model) return;
            
            applyCustomVoiceBtn.textContent = "SAVING...";
            applyCustomVoiceBtn.disabled = true;
            try {
                const res = await fetch("/api/voice/config", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                ...
                if (data.status === "success") {
                    let hasOption = false;
                    for (let i = 0; i < voiceModelSelect.options.length; i++) {
                        if (voiceModelSelect.options[i].value === voice_model) {
                            hasOption = true;
                            break;
                        }
                    }
                    if (!hasOption) {
                        const opt = document.createElement("option");
                        opt.value = voice_model;
                        opt.textContent = `${voice_model} (Custom)`;
                        voiceModelSelect.appendChild(opt);
                    }
                    voiceModelSelect.value = voice_model;
                }
            } catch (err) {
                console.error("Error applying custom voice:", err);
            } finally {
                applyCustomVoiceBtn.textContent = "Apply";
                applyCustomVoiceBtn.disabled = false;
            }
        });
    }
    ...
    if (uploadVoiceBtn) {
        uploadVoiceBtn.addEventListener("click", async () => {
            const modelFile = voiceOnnxUpload.files[0];
            const configFile = voiceJsonUpload.files[0];
            if (!modelFile || !configFile) {
                if (uploadVoiceStatus) {
                    uploadVoiceStatus.textContent = "Please select both .onnx and .onnx.json files.";
                    uploadVoiceStatus.style.color = "#ef4444";
                    uploadVoiceStatus.style.display = "block";
                }
                return;
            }
            ...
            try {
                ...
                if (res.ok && data.status === "success") {
                    if (uploadVoiceStatus) {
                        uploadVoiceStatus.textContent = `Successfully uploaded model "${data.model}"!`;
                        uploadVoiceStatus.style.color = "#10b981";
                        uploadVoiceStatus.style.display = "block";
                    }
                    await fetchLocalModels();
                    if (voiceModelSelect) {
                        let hasOption = false;
                        for (let i = 0; i < voiceModelSelect.options.length; i++) {
                            if (voiceModelSelect.options[i].value === data.model) {
                                hasOption = true;
                                break;
                            }
                        }
                        if (!hasOption) {
                            ...
                            opt.textContent = `${data.model} (Scanned Local)`;
                            localScannedVoices.appendChild(opt);
                        }
                        voiceModelSelect.value = data.model;
                        const voice_api_url = voiceApiUrlInput.value.trim();
                        await fetch("/api/voice/config", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ voice_api_url, voice_model: data.model })
                        });
                    }
                    voiceOnnxUpload.value = "";
                    voiceJsonUpload.value = "";
                } else {
                    throw new Error(data.message || "Upload failed");
                }
            } catch (err) {
                console.error("Error uploading voice files:", err);
                if (uploadVoiceStatus) {
                    uploadVoiceStatus.textContent = `Upload failed: ${err.message}`;
                    uploadVoiceStatus.style.color = "#ef4444";
                    uploadVoiceStatus.style.display = "block";
                }
            } finally {
                uploadVoiceBtn.textContent = "Upload & Register Voice";
    ...
    if (voiceSampleBtn) {
        voiceSampleBtn.addEventListener("click", async () => {
            const voice_model = voiceModelSelect ? voiceModelSelect.value : "en-US-AnaNeural";
            const sampleText = voiceSamples[voice_model] || "Hello! This is a voice preview.";
            
            voiceSampleBtn.textContent = "PLAYING...";
            voiceSampleBtn.disabled = true;
            voiceSampleBtn.style.opacity = "0.7";
            ...
            try {
                const res = await fetch("/api/voice/sample", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ text: sampleText, voice: voice_model })
                });
                await res.json();
            } catch (err) {
                console.error("Error triggering sample:", err);
            } finally {
                voiceSampleBtn.textContent = "SAMPLE";
                voiceSampleBtn.disabled = false;
                voiceSampleBtn.style.opacity = "1";
                if (typeof stopVoiceVisualizer === "function") {
                    stopVoiceVisualizer();
                }
            }
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener("click", async () => {
            if (confirm("Are you sure you want to log out of Cherry?")) {
                try {
